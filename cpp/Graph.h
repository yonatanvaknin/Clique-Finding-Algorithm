...
/// </summary>
 ...
 private:
	 ...
	 std::vector<VertexSet*> _V;
	 double _TH; // the threshold value
	 int _E_size;
	 bool _mat_flag;
 public:
	 Graph(const std::wstring &file, double th);
...
//			System.out.println("Edge: ["+curr.at(0)+","+curr.at(1)+"]");
	...
	/// </summary>
private:
	std::vector<VertexSet*> allEdges(); // all edges � all cliques of size 2/
	/// <summary>
	...
	/// </summary>
public:
	virtual std::vector<VertexSet*> All_Cliques_DFS(int min_size, int max_size);
	/// 
	/// <param name="min_size"> </param>
	/// <param name="max_size"> </param>
	 virtual void All_Cliques_DFS(const std::wstring &out_file, int min_size, int max_size);
	...
	/// <param name="C1"> </param>
private:
	void addToSet(std::vector<VertexSet*> &ans, std::vector<Clique*> &C1);
public:
	virtual std::vector<Clique*> allC_seed(Clique *edge, int min_size, int max_size);
